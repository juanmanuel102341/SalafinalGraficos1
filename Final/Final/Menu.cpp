#include"Menu.h"
bool Menu::collideEnter(sf::Sprite sprite) {
...
void Menu::detectStates(sf::Sprite& sprite, sf::Texture& textOut, sf::Texture& textOver, sf::Texture& textPress,bool& apply,currentState& _state,sf::Time delta, currentState objState ) {
	if (startState) {
		...
		if (timer > 2.5f) {
			...
			case currentState::credits:
				backMenu.setTexture(textureCredits);
				goCredits = true;
				timer = 0;
				startState = false;
				break;
...
void Menu::init(){
	...
	buttonPlay.setPosition(backMenu.getPosition().x + backMenu.getLocalBounds().width / 2 - buttonPlay.getLocalBounds().width / 2, buttonPlay.getLocalBounds().height + 120);
	...
	buttonMenu.setPosition(backMenu.getPosition().x + backMenu.getLocalBounds().width - buttonMenu.getLocalBounds().width , backMenu.getPosition().y + backMenu.getLocalBounds().height - buttonMenu.getLocalBounds().height);
}
Menu::Menu(sf::RenderWindow* _window): window(_window),mousseDown(false), applyTexture(false),press(false),startGame(false),timer(0),startState(false),goCredits(false),apply2(false),apply3(false),apply4(false) {
	init();
}
Menu::~Menu() {
